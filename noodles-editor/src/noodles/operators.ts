...
export class Tile3DLayerOp extends Operator<Tile3DLayerOp> {
  ...
  async execute(
    input: ExtractProps<typeof this.inputs>
  ): Promise<ExtractProps<typeof this.outputs>> {
    const {
      visible,
      flatLighting,
...
export class MVTLayerOp extends Operator<MVTLayerOp> {
  ...
  createInputs() {
    return {
      ...
      lineCapRounded: new BooleanField(false, { showByDefault: false }),
      lineJointRounded: new BooleanField(false, { showByDefault: false }),
      lineMiterLimit: new NumberField(4, { min: 0, showByDefault: false }),
      getFillColor: new ColorField('#000000', { accessor: true, transform: hexToColor }),
      getLineColor: new ColorField('#000000', { accessor: true, transform: hexToColor }),
      getLineWidth: new NumberField(1, { min: 0, accessor: true }),
      pointType: new StringLiteralField('circle', {
...
export class DataFilterOp extends Operator<DataFilterOp> {
  ...
  createInputs() {
    return {
      ...
      timeFilter: new StringLiteralField('all', {
        values: ['all', 'today', 'week', 'month', 'custom'],
      }),
  ...
  execute(props: ExtractProps<typeof this.inputs>): ExtractProps<typeof this.outputs> {
    ...
    switch (timeFilter) {
      case 'today':
        filterStart = now - 86400000
        break
      case 'week':
        filterStart = now - 604800000
        break
      case 'month':
        filterStart = now - 2592000000
        break
      case 'custom':
        filterStart = startTime || 0
        filterEnd = endTime || now
        break
    }

    let filtered = items.filter(item => {
      if (categoryFilter && (categoryFilter as string).trim()) {
        const types = (categoryFilter as string)
          .split(',')
      ...
      if (timeFilter !== 'all') {
        ...
        const itemEnd = (item.endTime ||
          (item.timestamps as number[])?.[((item.timestamps as number[])?.length ?? 1) - 1] ||
          0) as number
      ...
      if (useBounds && Array.isArray(item.path) && item.path.length > 0) {
        const inBounds = (item.path as [number, number][]).some(
...
export class ColorPaletteOp extends Operator<ColorPaletteOp> {
  ...
  execute(props: ExtractProps<typeof this.inputs>): ExtractProps<typeof this.outputs> {
    ...
    try {
      colorOverrides = JSON.parse((overridesJson as string) || '{}')
    } catch {
      /* ignore */
    }
...
export class DataRequestOp extends Operator<DataRequestOp> {
  ...
  execute(props: ExtractProps<typeof this.inputs>): ExtractProps<typeof this.outputs> {
    ...
    const parsedTypes = (activityTypes as string)
      .split(',')
      .map((t: string) => t.trim())
